...
def prepare_plot_and_calc_frames(
    ...
    # Only drop the last bar when it belongs to "today" (or later),
    # which is the common case for an intraday/unclosed bar.
    if exclude_last_bar_for_calc and len(df_calc) > 1:
        last_bar_date = pd.to_datetime(df_calc["date"].iloc[-1]).normalize()
